...
//            value = "SELECT * FROM estimateplussb.pricelistitem LEFT JOIN estimateplussb.pricelist_pricelistitem on pricelistitem.id = price_list_items_id left join estimateplussb.user on price_list_id = user_price_list_id where (user.user_price_list_id is null OR user.id = ?1) AND pricelistitem.reference_number LIKE ?2")
